(function () {
  ...
  const loadStatus = async () => {
    ...
    if (data?.isPremium) {
      ...
      if (payButton) {
        payButton.textContent = "Gia han bang MoMo";
      }
      return;
    }
  ...
  const createPayment = async () => {
    ...
    payButton.textContent = "Dang tao link MoMo...";
    ...
    try {
      ...
      if (!response.ok || !data?.success || !data?.payUrl) {
        throw new Error(data?.message || "MoMo chua tao duoc link thanh toan.");
      }

      window.location.href = data.payUrl;
    } catch (error) {
      showNotice(error.message || "Co loi khi tao thanh toan MoMo.", "error");
      payButton.disabled = false;
      payButton.textContent = "Thanh toan bang MoMo";
    }
  };
  ...
  const showReturnMessage = () => {
    ...
    if (payment === "success") {
      showNotice("MoMo da redirect ve thanh cong. He thong se mo Premium sau khi IPN duoc xac nhan.", "success");
    } else if (payment === "failed") {
      showNotice(params.get("message") || "Giao dich MoMo chua thanh cong.", "error");
    }
  };
